...
class ListProductPluginNew
{
    ...
    public function aroundGetProductDetailsHtml(
        ...
        $isEnable = $this->scopeConfig->getValue(
            self::XML_PATH_ENABLE_QUICKVIEW,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        if (!$isEnable) {
            return $result;
        }
        $buttonStyle = 'quickview forever-quickview forever-quickview-text' . $this->scopeConfig->getValue(
            self::XML_PATH_QUICKVIEW_BUTTONSTYLE,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $productUrl = $this->urlInterface->getUrl('quickview/catalog_product/view', ['id' => $product->getId()]);
        ...
        return $result . '<a 
        style="display: none" class="'.$buttonStyle.'"
        data-quickview-url="' . $productUrl . '"
        data-forever-js="' . $forever . '" href="' . $productUrl .'"
        ><span>' . __("Quickview") . '</span></a>'.$initializeHtml;
    }
}
