...
class File extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     */
    public function moveFileFromTmp($baseTmpPath, $basePath, $file, $newFileName)
    {
        if (isset($file)) {
            $fileName = $file;
            try {
                $fileName = $this->_moveFileFromTmp($baseTmpPath, $basePath, $file, $newFileName);
            } catch (\Exception $e) {
     ...
     * @return string
     */
    protected function _moveFileFromTmp($baseTmpPath, $basePath, $file, $newFileName)
    {
        if (strrpos($file, '.tmp') == strlen($file) - 4) {
            $file = substr($file, 0, strlen($file) - 4);
        }
        $pathInfo = $this->file->getPathInfo($file);
        $destFile = $pathInfo['dirname'] . '/' . $newFileName;
    ...
    public function getTmpFileUrl($file)
    {
        ...
        ) . \Ict\Attachments\Model\Data\File::BASE_TMP_PATH . $file;
        return $url;
    }
